...
std::vector<Track> CenterTrack::post_process()
{
    ...
    float *hm_data = hm->mutable_cpu_data();
    ...
    // for (int i = 0; i < 5; ++i)
    // {
    //     printf("%d_score: %3f\n", i, hm_vec[max_index[i]]);
    // }
    int channel_size = hm_w * hm_h;

    Blob<float> *reg = net_->blob_by_name("conv_blob26").get();
    const float *reg_data = reg->cpu_data();

    Blob<float> *wh = net_->blob_by_name("conv_blob28").get();
    const float *wh_data = wh->cpu_data();

    Blob<float> *track = net_->blob_by_name("conv_blob30").get();
    const float *track_data = track->cpu_data();

    // 下面三个变量用来坐标转换
    float scale = std::max(ori_w / 80., ori_h / 80.);
    // float delta_w = 0.;
    // float delta_h = 95.;
    float delta_w = (scale * 80 - ori_w) / 2.;
    float delta_h = (scale * 80 - ori_h) / 2.;
    ...
    // 从(hm_w, hm_h)到(ori_w, ori_h)的仿射变换矩阵
    // cv::Point2f src[3], dst[3];
    // src[0] = cv::Point2f(hm_w / 2., hm_h / 2.);
    // dst[0] = cv::Point2f(ori_w / 2., ori_h / 2.);
    // if (hm_w / hm_h < ori_w / ori_h)
    // {
    //     float delta_h = (hm_h - ori_h / float(ori_w) * hm_w) / 2.;
    //     src[1] = cv::Point2f(0., hm_h / 2.);
    //     dst[1] = cv::Point2f(0., ori_h / 2.);
    //     src[2] = cv::Point2f(0., delta_h);
    //     dst[2] = cv::Point2f(0., 0.);
    // }
    // else
    // {
    //     float delta_w = (hm_w - ori_w / float(ori_h) * hm_h) / 2.;
    //     src[1] = cv::Point2f(hm_w / 2., 0);
    //     dst[1] = cv::Point2f(ori_w / 2., 0);
    //     src[2] = cv::Point2f(delta_w, 0);
    //     dst[2] = cv::Point2f(0., 0.);
    // }
